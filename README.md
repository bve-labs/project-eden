...
* **Native `val_bpb` Calculation:** Because the tokens *are* bytes, standard cross-entropy loss cleanly converts to Validation Bits-Per-Byte (`loss / ln(2)`), providing mathematically pure scoring.
...
* PyTorch 2.0+
* NumPy
* *Hardware:* Apple Silicon (MPS) for local testing, or NVIDIA GPU (CUDA) for production scaling.
